/**
 * 100 bài tin tức SEO — batch 58 từ khóa ngắn (#5247–#5346).
...
import { shortKeywordCalendarBatch58 } from "../short-keyword-calendar-batch58";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch58
  ...
  const fallback = shortKeywordCalendarBatch58
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch58.map((item) => {
...
export const shortKeywordBatch58ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch58Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch58Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch58ArticlesBySlug[slug];
}
