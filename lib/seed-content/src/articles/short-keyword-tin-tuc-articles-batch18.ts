/**
 * 100 bài tin tức SEO — batch 18 từ khóa ngắn (#1247–#1346).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: mặt tiền, phong cách thiết kế, nhôm kính ngắn, sân vườn, geo vệ tinh, Ads intent.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch18 } from "../short-keyword-calendar-batch18";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (
    /quan-2|quan-4|quan-5|quan-6|quan-8|quan-11|thu-thiem|ben-cat|tan-uyen|long-thanh|nhon-trach|ben-luc|duc-hoa|quan-1|quan-3|quan-10|phu-nhuan|binh-thanh|thu-duc|long-an|go-vap/.test(
      s,
    )
  )
    return "geo";
  if (/bao-gia-nhom|bao-gia-nhanh|chi-phi|don-gia/.test(s)) return "pricing";
  if (/mien-phi|lien-he|hotline|gan-day|tim-nha|thue-thau|thue-thiet|dat-lich|nhan-bao|uy-tin|sao-khue/.test(s))
    return "turnkey";
  if (/nhom-kinh|xingfa|cua-nhom|cua-lua|cua-mo|cua-so|vach-kinh|mai-kinh|lan-can-kinh|mat-dung|bao-gia/.test(s))
    return "finish";
  if (/mat-tien|ngoai-that|op-da|op-gach|op-mat|son-mat|mai-hien|ban-cong|cong-rao|chieu-sang/.test(s))
    return "finish";
  if (/san-vuon|san-truoc|san-sau|tieu-canh|ho-ca|san-thuong|trong-cay|lat-san|duong-di|hang-rao-xanh|canh-quan|san-choi|bbq/.test(s))
    return "renovation";
  if (/phong-cach|hien-dai|toi-gian|indochine|nhat|bac-au|industrial|co-dien|tropical|skandi|luxury|xanh|thong-minh|mau-nha|xu-huong|thiet-ke/.test(s))
    return "design";
  if (/cai-tao|sua-nha/.test(s)) return "renovation";
  return "turnkey";
}

function detectLocation(slug: string, keyword: string): string {
  const s = `${slug} ${keyword}`.toLowerCase();
  if (/quan-2|quận 2|thu-thiem|thủ thiêm/.test(s)) return "Quận 2 / Thủ Thiêm, TP.HCM";
  if (/quan-4|quận 4/.test(s)) return "Quận 4, TP.HCM";
  if (/quan-5|quận 5/.test(s)) return "Quận 5, TP.HCM";
  if (/quan-6|quận 6/.test(s)) return "Quận 6, TP.HCM";
  if (/quan-8|quận 8/.test(s)) return "Quận 8, TP.HCM";
  if (/quan-11|quận 11/.test(s)) return "Quận 11, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch18
  ...
  const fallback = shortKeywordCalendarBatch18
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch18.map((item) => {
...
export const shortKeywordBatch18ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch18Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch18Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch18ArticlesBySlug[slug];
}
