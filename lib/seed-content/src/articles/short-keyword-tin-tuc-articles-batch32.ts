/**
 * 100 bài tin tức SEO — batch 32 từ khóa ngắn (#2647–#2746).
...
import { shortKeywordCalendarBatch32 } from "../short-keyword-calendar-batch32";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch32
  ...
  const fallback = shortKeywordCalendarBatch32
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch32.map((item) => {
...
export const shortKeywordBatch32ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch32Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch32Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch32ArticlesBySlug[slug];
}
