/**
 * 100 bài tin tức SEO — batch 15 từ khóa ngắn (#947–#1046).
...
import { shortKeywordCalendarBatch15 } from "../short-keyword-calendar-batch15";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (/binh-duong|dong-nai|thu-duc/.test(s)) return "geo";
  if (/sua-nha|sua-chua|sua-mai|sua-dien|son-nha/.test(s)) return "repair";
  if (/cai-tao|nang-tang|gia-co|chong-tham|lam-moi|mo-rong|mat-tien|noi-that|phong-bep/.test(s))
    return "renovation";
  if (/bao-gia|chi-phi|don-gia|gia-xay|du-toan|bang-gia|gia-xay-m2/.test(s)) return "pricing";
  if (/thiet-ke|mau-nha|ban-ve|kien-truc|3d|phong-khach/.test(s)) return "design";
  if (/phan-tho|mong-nha/.test(s)) return "rough";
  if (/1-tang|2-tang|3-tang|4-tang|5-tang|tret|lau|hem|dat-hep|mat-tien|tien-che|khung-thep|thang-may|san-vuon|cap-4|mini|lien-ke|ong|tro|xuong|van-phong/.test(s))
    return "floors";
  if (/xin-phep|giay-phep|giam-sat|quy-trinh|khao-sat|tu-van|nhan-thau/.test(s)) return "process";
  return "turnkey";
}
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch15
  ...
  const fallback = shortKeywordCalendarBatch15
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
function buildArticle(b: Brief): SeoArticleShape {
  ...
  const batchTopic = b.topic === "geo" ? "turnkey" : b.topic;
...
const briefs: Brief[] = shortKeywordCalendarBatch15.map((item) => {
...
export const shortKeywordBatch15ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch15Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch15Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch15ArticlesBySlug[slug];
}
