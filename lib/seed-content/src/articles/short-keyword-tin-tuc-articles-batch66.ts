/**
 * 100 bài tin tức SEO — batch 66 từ khóa ngắn (#6047–#6146).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk66).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch66 } from "../short-keyword-calendar-batch66";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/bau-bang|bàu bàng/.test(s)) return "Bàu Bàng, Bình Dương";
  ...
  if (/thu-dau-mot|thủ dầu một/.test(s)) return "Thủ Dầu Một, Bình Dương";
  ...
  if (/linh-trung|tam-phu|phu-huu|long-truong|phuoc-long|quan-2|quan-9|thu-duc|thủ đức/.test(s))
  ...
  if (/binh-tan|bình tân|binh-hung|an-lac|tan-tao/.test(s)) return "Bình Tân, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch66
  ...
  const fallback = shortKeywordCalendarBatch66
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch66.map((item) => {
...
export const shortKeywordBatch66ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch66Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch66Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch66ArticlesBySlug[slug];
}
