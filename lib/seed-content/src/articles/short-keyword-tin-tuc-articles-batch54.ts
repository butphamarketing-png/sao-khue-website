/**
 * 100 bài tin tức SEO — batch 54 từ khóa ngắn (#4847–#4946).
...
import { shortKeywordCalendarBatch54 } from "../short-keyword-calendar-batch54";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch54
  ...
  const fallback = shortKeywordCalendarBatch54
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch54.map((item) => {
...
export const shortKeywordBatch54ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch54Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch54Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch54ArticlesBySlug[slug];
}
