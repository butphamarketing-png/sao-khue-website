/**
 * 100 bài tin tức SEO — batch 71 từ khóa ngắn (#6547–#6646).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk71).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch71 } from "../short-keyword-calendar-batch71";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/quan-8|kenh-te|cho-lon/.test(s)) return "Quận 8, TP.HCM";
  if (/quan-12|dong-hung|tan-chanh|tan-thoi|thoi-an/.test(s)) return "Quận 12, TP.HCM";
  if (/binh-tan|bình tân|an-lac|binh-hung|tan-tao|aeon|ten-lua/.test(s)) return "Bình Tân, TP.HCM";
  if (/thu-duc|thủ đức/.test(s)) return "Thủ Đức, TP.HCM";
  if (/quan-7/.test(s)) return "Quận 7, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch71
  ...
  const fallback = shortKeywordCalendarBatch71
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch71.map((item) => {
...
export const shortKeywordBatch71ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch71Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch71Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch71ArticlesBySlug[slug];
}
