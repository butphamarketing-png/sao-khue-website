/**
 * 100 bài tin tức SEO — batch 45 từ khóa ngắn (#3947–#4046).
...
import { shortKeywordCalendarBatch45 } from "../short-keyword-calendar-batch45";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch45
  ...
  const fallback = shortKeywordCalendarBatch45
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch45.map((item) => {
...
export const shortKeywordBatch45ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch45Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch45Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch45ArticlesBySlug[slug];
}
