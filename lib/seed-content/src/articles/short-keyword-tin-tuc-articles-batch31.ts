/**
 * 100 bài tin tức SEO — batch 31 từ khóa ngắn (#2547–#2646).
...
import { shortKeywordCalendarBatch31 } from "../short-keyword-calendar-batch31";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch31
  ...
  const fallback = shortKeywordCalendarBatch31
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch31.map((item) => {
...
export const shortKeywordBatch31ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch31Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch31Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch31ArticlesBySlug[slug];
}
