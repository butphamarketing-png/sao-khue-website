/**
 * 100 bài tin tức SEO — batch 51 từ khóa ngắn (#4547–#4646).
...
import { shortKeywordCalendarBatch51 } from "../short-keyword-calendar-batch51";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch51
  ...
  const fallback = shortKeywordCalendarBatch51
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch51.map((item) => {
...
export const shortKeywordBatch51ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch51Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch51Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch51ArticlesBySlug[slug];
}
