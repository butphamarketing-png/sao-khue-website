/**
 * 100 bài tin tức SEO — batch 70 từ khóa ngắn (#6447–#6546).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk70).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch70 } from "../short-keyword-calendar-batch70";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (
    /thao-dien|an-phu|binh-trung|cat-lai|hiep-phu|tang-nhon|phuoc-long|tan-thuan|binh-thuan-quan|di-an|thuan-an|bien-hoa|long-thanh|ben-luc|can-giuoc|thu-dau-mot|long-an|dong-nai|binh-duong|tan-uyen|bau-bang|nhon-trach|trang-bom|duc-hoa|can-duoc|ben-cat|hoc-mon|cu-chi|binh-chanh|nha-be|linh-trung|tam-phu|phu-huu|long-truong|binh-hung|an-lac|tan-tao|phuong-21|phuong-26|phuong-7|phuong-15|phuong-4|phuong-12|phuong-8|quan-2|quan-9/.test(
      s,
    )
  )
    return "geo";
  if (/bao-gia|gia-xay|don-gia|bang-gia|chi-phi|2026|ngan-sach|nhan-cong|phat-sinh|du-toan|thanh-toan/.test(s))
    return "pricing";
  if (/thiet-ke|mau-nha|noi-that|phong-khach|phong-ngu|indochine|toi-gian|mai-nhat|mai-thai|biet-thu|nha-vuon/.test(s))
    return "design";
  if (/mong-|do-mong|do-san|ep-coc|ket-cau|gia-co|khung-thep|be-tong|cot-thep/.test(s))
    return "rough";
  if (/mai-|lam-mai|cach-nhiet|tran-thach|son-ngoai|op-da|lat-gach|nhom-kinh|cua-nhom|dien-3-pha|nuoc-nong/.test(s))
    return "finish";
  if (/sua-|chong-tham|tham-tran|nut-cot/.test(s)) return "repair";
  if (/cai-tao|nang-tang/.test(s)) return "renovation";
  if (/checklist|chon-nha-thau|nghiem-thu|bao-hanh|ky-hop-dong|cach-doc/.test(s)) return "process";
  if (/xay-nha-1-tret|xay-nha-5x|xay-nha-mini|thang-may|lien-ke|hem-/.test(s)) return "floors";
  return "turnkey";
}

function detectLocation(slug: string, keyword: string): string {
  const s = `${slug} ${keyword}`.toLowerCase();
  if (/can-duoc|cần đước/.test(s)) return "Cần Đước, Long An";
  if (/tan-uyen|tân uyên/.test(s)) return "Tân Uyên, Bình Dương";
  if (/ben-cat|bến cát/.test(s)) return "Bến Cát, Bình Dương";
  if (/long-thanh|long thành/.test(s)) return "Long Thành, Đồng Nai";
  if (/nhon-trach|nhơn trạch/.test(s)) return "Nhơn Trạch, Đồng Nai";
  if (/di-an|dĩ an/.test(s)) return "Dĩ An, Bình Dương";
  if (/thuan-an|thuận an/.test(s)) return "Thuận An, Bình Dương";
  if (/ben-luc|bến lức/.test(s)) return "Bến Lức, Long An";
  if (/duc-hoa|đức hòa/.test(s)) return "Đức Hòa, Long An";
  if (/can-giuoc|cần giuộc/.test(s)) return "Cần Giuộc, Long An";
  if (/bien-hoa|biên hòa/.test(s)) return "Biên Hòa, Đồng Nai";
  if (/dong-nai|đồng nai/.test(s)) return "Đồng Nai";
  if (/long-an/.test(s)) return "Long An";
  if (/binh-duong|bình dương/.test(s)) return "Bình Dương";
  if (/cu-chi|củ chi/.test(s)) return "Củ Chi, TP.HCM";
  if (/hoc-mon|hóc môn/.test(s)) return "Hóc Môn, TP.HCM";
  if (/binh-chanh|bình chánh/.test(s)) return "Bình Chánh, TP.HCM";
  if (/nha-be|nhà bè/.test(s)) return "Nhà Bè, TP.HCM";
  if (
    /thu-duc|thủ đức|binh-chieu|hiep-binh|linh-chieu|linh-dong|linh-tay|truong-tho|tam-binh-thu|tan-phu-thu|landmark|grand-park|suoi-tien|quoc-gia/.test(
      s,
    )
  )
    return "Thủ Đức, TP.HCM";
  if (/quan-7|phu-my|tan-phong|tan-quy-quan|tan-kieng|phu-thuan|phu-my-hung/.test(s))
    return "Quận 7, TP.HCM";
  if (/go-vap|gò vấp/.test(s)) return "Gò Vấp, TP.HCM";
  if (/binh-thanh|bình thạnh/.test(s)) return "Bình Thạnh, TP.HCM";
  if (/phu-nhuan|phú nhuận/.test(s)) return "Phú Nhuận, TP.HCM";
  if (/tan-binh|tân bình/.test(s)) return "Tân Bình, TP.HCM";
  if (/tan-phu|tân phú/.test(s)) return "Tân Phú, TP.HCM";
  if (/binh-tan|bình tân/.test(s)) return "Bình Tân, TP.HCM";
  return "TP.HCM";
}
...
<p><strong>Kiến Trúc Sao Khuê</strong> (245/8 Bình Lợi, Bình Thạnh) triển khai <strong>${kw}</strong> theo hướng: khảo sát miễn phí → dự toán chi tiết → thi công trực tiếp → bàn giao bảo hành kết cấu <strong>10 năm</strong>.</p>`,
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch70
  ...
  const fallback = shortKeywordCalendarBatch70
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
<p>Địa chỉ liên hệ nhanh: 245/8 Bình Lợi, Bình Thạnh — hotline 0909 075 668.</p>`,
  ];
...
const briefs: Brief[] = shortKeywordCalendarBatch70.map((item) => {
...
export const shortKeywordBatch70ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch70Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch70Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch70ArticlesBySlug[slug];
}
