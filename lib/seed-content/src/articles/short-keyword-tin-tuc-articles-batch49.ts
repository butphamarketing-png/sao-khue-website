/**
 * 100 bài tin tức SEO — batch 49 từ khóa ngắn (#4347–#4446).
...
import { shortKeywordCalendarBatch49 } from "../short-keyword-calendar-batch49";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch49
  ...
  const fallback = shortKeywordCalendarBatch49
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch49.map((item) => {
...
export const shortKeywordBatch49ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch49Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch49Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch49ArticlesBySlug[slug];
}
