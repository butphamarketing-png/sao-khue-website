/**
 * 100 bài tin tức SEO — batch 25 từ khóa ngắn (#1947–#2046).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: nhà lỗi/hư hỏng, khắc phục thẩm thấu/nứt/ẩm, sửa chữa chuyên đề.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch25 } from "../short-keyword-calendar-batch25";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch25
  ...
  const fallback = shortKeywordCalendarBatch25
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch25.map((item) => {
...
export const shortKeywordBatch25ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch25Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch25Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch25ArticlesBySlug[slug];
}
