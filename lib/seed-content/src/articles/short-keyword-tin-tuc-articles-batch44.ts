/**
 * 100 bài tin tức SEO — batch 44 từ khóa ngắn (#3847–#3946).
...
import { shortKeywordCalendarBatch44 } from "../short-keyword-calendar-batch44";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch44
  ...
  const fallback = shortKeywordCalendarBatch44
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch44.map((item) => {
...
export const shortKeywordBatch44ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch44Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch44Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch44ArticlesBySlug[slug];
}
