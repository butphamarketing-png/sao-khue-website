/**
 * 100 bài tin tức SEO — batch 26 từ khóa ngắn (#2047–#2146).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: kích thước/tầng/mẫu nhà, diện tích, mặt tiền, số tầng.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch26 } from "../short-keyword-calendar-batch26";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch26
  ...
  const fallback = shortKeywordCalendarBatch26
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch26.map((item) => {
...
export const shortKeywordBatch26ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch26Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch26Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch26ArticlesBySlug[slug];
}
