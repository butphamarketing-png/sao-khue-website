/**
 * 100 bài tin tức SEO — batch 59 từ khóa ngắn (#5347–#5446).
...
import { shortKeywordCalendarBatch59 } from "../short-keyword-calendar-batch59";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch59
  ...
  const fallback = shortKeywordCalendarBatch59
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch59.map((item) => {
...
export const shortKeywordBatch59ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch59Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch59Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch59ArticlesBySlug[slug];
}
