/**
 * 100 bài tin tức SEO — batch 35 từ khóa ngắn (#2947–#3046).
...
import { shortKeywordCalendarBatch35 } from "../short-keyword-calendar-batch35";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch35
  ...
  const fallback = shortKeywordCalendarBatch35
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch35.map((item) => {
...
export const shortKeywordBatch35ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch35Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch35Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch35ArticlesBySlug[slug];
}
