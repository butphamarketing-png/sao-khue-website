/**
 * 100 bài tin tức SEO — batch 61 từ khóa ngắn (#5547–#5646).
...
import { shortKeywordCalendarBatch61 } from "../short-keyword-calendar-batch61";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch61
  ...
  const fallback = shortKeywordCalendarBatch61
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch61.map((item) => {
...
export const shortKeywordBatch61ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch61Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch61Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch61ArticlesBySlug[slug];
}
