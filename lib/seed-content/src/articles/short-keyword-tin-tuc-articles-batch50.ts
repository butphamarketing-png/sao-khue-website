/**
 * 100 bài tin tức SEO — batch 50 từ khóa ngắn (#4447–#4546).
...
import { shortKeywordCalendarBatch50 } from "../short-keyword-calendar-batch50";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch50
  ...
  const fallback = shortKeywordCalendarBatch50
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch50.map((item) => {
...
export const shortKeywordBatch50ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch50Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch50Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch50ArticlesBySlug[slug];
}
