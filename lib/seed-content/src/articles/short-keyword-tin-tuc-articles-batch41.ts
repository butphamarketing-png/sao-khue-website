/**
 * 100 bài tin tức SEO — batch 41 từ khóa ngắn (#3547–#3646).
...
import { shortKeywordCalendarBatch41 } from "../short-keyword-calendar-batch41";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch41
  ...
  const fallback = shortKeywordCalendarBatch41
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch41.map((item) => {
...
export const shortKeywordBatch41ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch41Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch41Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch41ArticlesBySlug[slug];
}
