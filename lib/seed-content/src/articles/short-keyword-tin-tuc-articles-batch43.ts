/**
 * 100 bài tin tức SEO — batch 43 từ khóa ngắn (#3747–#3846).
...
import { shortKeywordCalendarBatch43 } from "../short-keyword-calendar-batch43";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch43
  ...
  const fallback = shortKeywordCalendarBatch43
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch43.map((item) => {
...
export const shortKeywordBatch43ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch43Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch43Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch43ArticlesBySlug[slug];
}
