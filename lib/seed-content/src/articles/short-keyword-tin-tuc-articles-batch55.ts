/**
 * 100 bài tin tức SEO — batch 55 từ khóa ngắn (#4947–#5046).
...
import { shortKeywordCalendarBatch55 } from "../short-keyword-calendar-batch55";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch55
  ...
  const fallback = shortKeywordCalendarBatch55
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch55.map((item) => {
...
export const shortKeywordBatch55ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch55Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch55Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch55ArticlesBySlug[slug];
}
