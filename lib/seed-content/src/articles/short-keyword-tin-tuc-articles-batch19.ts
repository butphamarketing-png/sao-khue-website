...
import { shortKeywordCalendarBatch19 } from "../short-keyword-calendar-batch19";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch19
  ...
  const fallback = shortKeywordCalendarBatch19
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch19.map((item) => {
...
export const shortKeywordBatch19ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch19Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch19Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch19ArticlesBySlug[slug];
}
