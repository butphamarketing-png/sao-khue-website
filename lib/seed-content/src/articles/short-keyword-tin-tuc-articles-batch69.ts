/**
 * 100 bài tin tức SEO — batch 69 từ khóa ngắn (#6347–#6446).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk69).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch69 } from "../short-keyword-calendar-batch69";
...
function detectLocation(slug: string, keyword: string): string {
  const s = `${slug} ${keyword}`.toLowerCase();
  if (/cam-my|cẩm mỹ/.test(s)) return "Cẩm Mỹ, Đồng Nai";
  if (/xuan-loc|xuân lộc/.test(s)) return "Xuân Lộc, Đồng Nai";
  if (/dinh-quan|định quán/.test(s)) return "Định Quán, Đồng Nai";
  if (/chau-thanh|châu thành/.test(s)) return "Châu Thành, Long An";
  if (/thu-thua|thủ thừa/.test(s)) return "Thủ Thừa, Long An";
  if (/tan-an|tân an/.test(s)) return "Tân An, Long An";
  if (/bac-tan-uyen|bắc tân uyên/.test(s)) return "Bắc Tân Uyên, Bình Dương";
  ...
  if (/go-vap|gò vấp|phan-van-tri|quang-trung/.test(s)) return "Gò Vấp, TP.HCM";
  if (/binh-thanh|bình thạnh|xo-viet|dinh-bo-linh/.test(s)) return "Bình Thạnh, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch69
  ...
  const fallback = shortKeywordCalendarBatch69
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch69.map((item) => {
...
export const shortKeywordBatch69ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch69Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch69Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch69ArticlesBySlug[slug];
}
