/**
 * 100 bài tin tức SEO — batch 29 từ khóa ngắn (#2347–#2446).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: quyết định mua hàng, FAQ, quy trình làm việc với chủ đầu tư.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch29 } from "../short-keyword-calendar-batch29";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch29
  ...
  const fallback = shortKeywordCalendarBatch29
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch29.map((item) => {
...
export const shortKeywordBatch29ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch29Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch29Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch29ArticlesBySlug[slug];
}
