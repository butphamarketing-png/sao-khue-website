...
import { shortKeywordCalendarBatch20 } from "../short-keyword-calendar-batch20";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch20
  ...
  const fallback = shortKeywordCalendarBatch20
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch20.map((item) => {
...
export const shortKeywordBatch20ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch20Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch20Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch20ArticlesBySlug[slug];
}
