/**
 * 100 bài tin tức SEO — batch 72 từ khóa ngắn (#6647–#6746).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk72).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch72 } from "../short-keyword-calendar-batch72";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/quan-4|khanh-hoi/.test(s)) return "Quận 4, TP.HCM";
  if (/quan-5|an-dong|cho-an-dong/.test(s)) return "Quận 5, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch72
  ...
  const fallback = shortKeywordCalendarBatch72
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch72.map((item) => {
...
export const shortKeywordBatch72ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch72Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch72Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch72ArticlesBySlug[slug];
}
