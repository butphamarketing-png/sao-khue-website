/**
 * 100 bài tin tức SEO — batch 33 từ khóa ngắn (#2747–#2846).
...
import { shortKeywordCalendarBatch33 } from "../short-keyword-calendar-batch33";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch33
  ...
  const fallback = shortKeywordCalendarBatch33
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch33.map((item) => {
...
export const shortKeywordBatch33ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch33Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch33Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch33ArticlesBySlug[slug];
}
