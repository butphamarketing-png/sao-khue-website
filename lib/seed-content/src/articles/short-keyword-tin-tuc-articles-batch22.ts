...
import { shortKeywordCalendarBatch22 } from "../short-keyword-calendar-batch22";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch22
  ...
  const fallback = shortKeywordCalendarBatch22
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch22.map((item) => {
...
export const shortKeywordBatch22ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch22Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch22Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch22ArticlesBySlug[slug];
}
