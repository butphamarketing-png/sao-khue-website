/**
 * 100 bài tin tức SEO — batch 40 từ khóa ngắn (#3447–#3546).
...
import { shortKeywordCalendarBatch40 } from "../short-keyword-calendar-batch40";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch40
  ...
  const fallback = shortKeywordCalendarBatch40
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch40.map((item) => {
...
export const shortKeywordBatch40ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch40Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch40Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch40ArticlesBySlug[slug];
}
