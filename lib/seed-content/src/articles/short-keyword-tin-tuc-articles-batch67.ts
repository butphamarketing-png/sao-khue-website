/**
 * 100 bài tin tức SEO — batch 67 từ khóa ngắn (#6147–#6246).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk67).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch67 } from "../short-keyword-calendar-batch67";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/trang-bang|trảng bàng/.test(s)) return "Trảng Bàng, Tây Ninh";
  if (/phu-giao|phú giáo/.test(s)) return "Phú Giáo, Bình Dương";
  ...
  if (/binh-duong|bình dương|cu-chi|củ chi/.test(s)) return /cu-chi|củ chi/.test(s) ? "Củ Chi, TP.HCM" : "Bình Dương";
  if (/quan-1|bến nghé|ben-nghe|da-kao|đa kao/.test(s)) return "Quận 1, TP.HCM";
  if (/quan-3|cau-kho|nguyen-cu-trinh/.test(s)) return "Quận 3, TP.HCM";
  if (/quan-4/.test(s)) return "Quận 4, TP.HCM";
  if (/quan-5/.test(s)) return "Quận 5, TP.HCM";
  if (/quan-6/.test(s)) return "Quận 6, TP.HCM";
  if (/quan-8/.test(s)) return "Quận 8, TP.HCM";
  if (/quan-10/.test(s)) return "Quận 10, TP.HCM";
  if (/quan-11/.test(s)) return "Quận 11, TP.HCM";
  if (/quan-12|hiep-thanh|trung-my-tay|thanh-xuan|an-phu-dong/.test(s)) return "Quận 12, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch67
  ...
  const fallback = shortKeywordCalendarBatch67
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch67.map((item) => {
...
export const shortKeywordBatch67ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch67Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch67Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch67ArticlesBySlug[slug];
}
