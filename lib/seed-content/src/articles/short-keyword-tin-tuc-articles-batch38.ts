/**
 * 100 bài tin tức SEO — batch 38 từ khóa ngắn (#3247–#3346).
...
import { shortKeywordCalendarBatch38 } from "../short-keyword-calendar-batch38";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch38
  ...
  const fallback = shortKeywordCalendarBatch38
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch38.map((item) => {
...
export const shortKeywordBatch38ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch38Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch38Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch38ArticlesBySlug[slug];
}
