/**
 * 100 bài tin tức SEO — batch 64 từ khóa ngắn (#5847–#5946).
...
import { shortKeywordCalendarBatch64 } from "../short-keyword-calendar-batch64";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch64
  ...
  const fallback = shortKeywordCalendarBatch64
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch64.map((item) => {
...
export const shortKeywordBatch64ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch64Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch64Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch64ArticlesBySlug[slug];
}
