/**
 * 100 bài tin tức SEO — batch 48 từ khóa ngắn (#4247–#4346).
...
import { shortKeywordCalendarBatch48 } from "../short-keyword-calendar-batch48";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch48
  ...
  const fallback = shortKeywordCalendarBatch48
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch48.map((item) => {
...
export const shortKeywordBatch48ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch48Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch48Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch48ArticlesBySlug[slug];
}
