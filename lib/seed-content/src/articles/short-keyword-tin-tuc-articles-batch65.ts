/**
 * 100 bài tin tức SEO — batch 65 từ khóa ngắn (#5947–#6046).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk65).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch65 } from "../short-keyword-calendar-batch65";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (
    /thao-dien|an-phu|binh-trung|cat-lai|hiep-phu|tang-nhon|phuoc-long|tan-thuan|binh-thuan-quan|di-an|thuan-an|bien-hoa|long-thanh|ben-luc|can-giuoc|thu-dau-mot|long-an|dong-nai|binh-duong/.test(
      s,
    )
  )
    return "geo";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/thao-dien|thảo điền|an-phu|binh-trung|cat-lai|hiep-phu|tang-nhon|phuoc-long|thu-duc|thủ đức/.test(s))
    return "Thủ Đức, TP.HCM";
  if (/quan-7|quận 7|tan-thuan|binh-thuan-quan/.test(s)) return "Quận 7, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch65
  ...
  const fallback = shortKeywordCalendarBatch65
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch65.map((item) => {
...
export const shortKeywordBatch65ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch65Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch65Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch65ArticlesBySlug[slug];
}
