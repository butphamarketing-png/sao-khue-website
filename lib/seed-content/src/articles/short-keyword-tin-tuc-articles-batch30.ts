/**
 * 100 bài tin tức SEO — batch 30 từ khóa ngắn (#2447–#2546).
...
import { shortKeywordCalendarBatch30 } from "../short-keyword-calendar-batch30";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch30
  ...
  const fallback = shortKeywordCalendarBatch30
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch30.map((item) => {
...
export const shortKeywordBatch30ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch30Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch30Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch30ArticlesBySlug[slug];
}
