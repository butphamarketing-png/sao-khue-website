/**
 * 100 bài tin tức SEO — batch 57 từ khóa ngắn (#5147–#5246).
...
import { shortKeywordCalendarBatch57 } from "../short-keyword-calendar-batch57";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch57
  ...
  const fallback = shortKeywordCalendarBatch57
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch57.map((item) => {
...
export const shortKeywordBatch57ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch57Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch57Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch57ArticlesBySlug[slug];
}
