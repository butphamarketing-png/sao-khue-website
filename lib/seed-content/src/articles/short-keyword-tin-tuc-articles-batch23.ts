...
import { shortKeywordCalendarBatch23 } from "../short-keyword-calendar-batch23";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch23
  ...
  const fallback = shortKeywordCalendarBatch23
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch23.map((item) => {
...
export const shortKeywordBatch23ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch23Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch23Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch23ArticlesBySlug[slug];
}
