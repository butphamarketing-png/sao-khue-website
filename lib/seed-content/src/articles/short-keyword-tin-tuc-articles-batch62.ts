/**
 * 100 bài tin tức SEO — batch 62 từ khóa ngắn (#5647–#5746).
...
import { shortKeywordCalendarBatch62 } from "../short-keyword-calendar-batch62";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch62
  ...
  const fallback = shortKeywordCalendarBatch62
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch62.map((item) => {
...
export const shortKeywordBatch62ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch62Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch62Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch62ArticlesBySlug[slug];
}
