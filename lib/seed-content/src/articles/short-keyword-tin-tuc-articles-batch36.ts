/**
 * 100 bài tin tức SEO — batch 36 từ khóa ngắn (#3047–#3146).
...
import { shortKeywordCalendarBatch36 } from "../short-keyword-calendar-batch36";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch36
  ...
  const fallback = shortKeywordCalendarBatch36
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch36.map((item) => {
...
export const shortKeywordBatch36ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch36Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch36Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch36ArticlesBySlug[slug];
}
