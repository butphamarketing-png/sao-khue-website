...
import { shortKeywordCalendarBatch24 } from "../short-keyword-calendar-batch24";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch24
  ...
  const fallback = shortKeywordCalendarBatch24
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch24.map((item) => {
...
export const shortKeywordBatch24ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch24Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch24Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch24ArticlesBySlug[slug];
}
