/**
 * 100 bài tin tức SEO — batch 53 từ khóa ngắn (#4747–#4846).
...
import { shortKeywordCalendarBatch53 } from "../short-keyword-calendar-batch53";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch53
  ...
  const fallback = shortKeywordCalendarBatch53
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch53.map((item) => {
...
export const shortKeywordBatch53ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch53Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch53Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch53ArticlesBySlug[slug];
}
