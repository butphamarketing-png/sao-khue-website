/**
 * 100 bài tin tức SEO — batch 37 từ khóa ngắn (#3147–#3246).
...
import { shortKeywordCalendarBatch37 } from "../short-keyword-calendar-batch37";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch37
  ...
  const fallback = shortKeywordCalendarBatch37
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch37.map((item) => {
...
export const shortKeywordBatch37ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch37Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch37Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch37ArticlesBySlug[slug];
}
