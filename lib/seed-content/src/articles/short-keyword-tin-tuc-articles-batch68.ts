/**
 * 100 bài tin tức SEO — batch 68 từ khóa ngắn (#6247–#6346).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk68).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch68 } from "../short-keyword-calendar-batch68";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/tan-binh|tân bình|tan-son-nhat|sân bay/.test(s)) return "Tân Bình, TP.HCM";
  if (/tan-phu|tân phú|tay-thanh|son-ky|tan-quy|hiep-tan/.test(s)) return "Tân Phú, TP.HCM";
  if (/binh-tan|bình tân|binh-tri-dong/.test(s)) return "Bình Tân, TP.HCM";
  if (/quan-1|bến nghé|ben-nghe|da-kao/.test(s)) return "Quận 1, TP.HCM";
  if (/quan-3/.test(s)) return "Quận 3, TP.HCM";
  if (/quan-12/.test(s)) return "Quận 12, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch68
  ...
  const fallback = shortKeywordCalendarBatch68
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch68.map((item) => {
...
export const shortKeywordBatch68ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch68Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch68Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch68ArticlesBySlug[slug];
}
