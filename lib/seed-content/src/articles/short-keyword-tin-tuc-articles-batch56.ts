/**
 * 100 bài tin tức SEO — batch 56 từ khóa ngắn (#5047–#5146).
...
import { shortKeywordCalendarBatch56 } from "../short-keyword-calendar-batch56";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch56
  ...
  const fallback = shortKeywordCalendarBatch56
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch56.map((item) => {
...
export const shortKeywordBatch56ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch56Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch56Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch56ArticlesBySlug[slug];
}
