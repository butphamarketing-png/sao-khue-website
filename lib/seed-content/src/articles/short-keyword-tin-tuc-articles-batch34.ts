/**
 * 100 bài tin tức SEO — batch 34 từ khóa ngắn (#2847–#2946).
...
import { shortKeywordCalendarBatch34 } from "../short-keyword-calendar-batch34";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch34
  ...
  const fallback = shortKeywordCalendarBatch34
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch34.map((item) => {
...
export const shortKeywordBatch34ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch34Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch34Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch34ArticlesBySlug[slug];
}
