/**
 * 100 bài tin tức SEO — batch 39 từ khóa ngắn (#3347–#3446).
...
import { shortKeywordCalendarBatch39 } from "../short-keyword-calendar-batch39";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch39
  ...
  const fallback = shortKeywordCalendarBatch39
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch39.map((item) => {
...
export const shortKeywordBatch39ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch39Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch39Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch39ArticlesBySlug[slug];
}
