/**
 * 100 bài tin tức SEO — batch 16 từ khóa ngắn (#1047–#1146).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: hoàn thiện/vật liệu, nội thất, cải tạo chi tiết, pháp lý, geo, Ads.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch16 } from "../short-keyword-calendar-batch16";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (
    /go-vap|binh-thanh|quan-7|quan-9|tan-binh|tan-phu|binh-chanh|hoc-mon|nha-be|quan-12|long-an|bien-hoa|thuan-an|di-an|thu-dau-mot|buon-ma-thuot|thu-duc/.test(
      s,
    )
  )
    return "geo";
  if (/bao-gia|chi-phi|don-gia|gia-cai|bang-gia|gia-re/.test(s)) return "pricing";
  if (/sua-tuong|sua-nen|sua-nha|xu-ly-nha|xu-ly/.test(s)) return "repair";
  if (
    /cai-tao|nang-nen|gia-co|dap-thong|mo-rong|lam-them|tang-lung|tang-ap-mai|san-thuong|ban-cong/.test(
      s,
    )
  )
    return "renovation";
  if (/thiet-ke-phong|thiet-ke-ban|thiet-ke-san|thiet-ke-gieng|thiet-ke-cau|thiet-ke-gara/.test(s))
    return "design";
  if (/noi-that|tu-quan|vach-ngan|lam-bep|phong-ngu|phong-wc/.test(s)) return "interior";
  if (
    /hoan-thien|op-lat|lat-gach|son-nuoc|tran-thach|lam-cua|cua-nhom|san-go|giay-dan|den-nha|tu-bep|ve-sinh|lan-can|cong-nha|hang-rao|mai-ton|lam-mai|chong-nong|cach-am/.test(
      s,
    )
  )
    return "finish";
  if (
    /hoan-cong|so-hong|thu-tuc|quy-hoach|chi-gioi|xin-phep|giay-phep|ho-so-xin/.test(s)
  )
    return "process";
  return "turnkey";
}
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch16
  ...
  const fallback = shortKeywordCalendarBatch16
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch16.map((item) => {
...
export const shortKeywordBatch16ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch16Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch16Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch16ArticlesBySlug[slug];
}
