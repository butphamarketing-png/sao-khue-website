/**
 * 100 bài tin tức SEO — batch 46 từ khóa ngắn (#4047–#4146).
...
import { shortKeywordCalendarBatch46 } from "../short-keyword-calendar-batch46";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch46
  ...
  const fallback = shortKeywordCalendarBatch46
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch46.map((item) => {
...
export const shortKeywordBatch46ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch46Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch46Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch46ArticlesBySlug[slug];
}
