/**
 * 100 bài tin tức SEO — batch 60 từ khóa ngắn (#5447–#5546).
...
import { shortKeywordCalendarBatch60 } from "../short-keyword-calendar-batch60";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch60
  ...
  const fallback = shortKeywordCalendarBatch60
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch60.map((item) => {
...
export const shortKeywordBatch60ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch60Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch60Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch60ArticlesBySlug[slug];
}
