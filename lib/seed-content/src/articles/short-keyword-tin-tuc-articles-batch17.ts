/**
 * 100 bài tin tức SEO — batch 17 từ khóa ngắn (#1147–#1246).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: chống thấm/MEP, ngân sách, nâng tầng, geo mở rộng, kinh nghiệm chọn thầu.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch17 } from "../short-keyword-calendar-batch17";
...
function detectTopic(slug: string, keyword: string): Topic {
  ...
  if (
    /go-vap|binh-thanh|quan-1|quan-3|quan-7|quan-9|quan-10|phu-nhuan|binh-tan|cu-chi|can-gio|tan-binh|tan-phu|binh-chanh|hoc-mon|nha-be|quan-12|long-an|bien-hoa|thuan-an|di-an|thu-dau-mot|buon-ma-thuot|thu-duc|phu-yen|tuy-hoa|dak-lak|binh-duong|dong-nai/.test(
      s,
    )
  )
    return "geo";
  if (/bao-gia|chi-phi|don-gia|gia-cai|bang-gia|gia-re|trieu|1-ty|2-ty|3-ty|ngan-sach|m2|dien-tich/.test(s))
    return "pricing";
  if (/chong-tham|tham-dot|tham-nuoc|nut-tuong|nut-tran|bao-tri|bao-duong|sua-ong|sua-nha/.test(s))
    return "repair";
  if (/nang-tang|nang-them|coi-noi|xay-them|gia-co-truoc|cai-tao/.test(s)) return "renovation";
  if (/dien-nuoc|he-thong-dien|he-thong-nuoc|lap-dat-dien|lap-dat-nuoc|bom-nuoc|binh-nong|may-lanh|thong-gio|hut-mui|camera|khoa-cong|pccc/.test(s))
    return "finish";
  if (/xin-phep|giay-phep|hop-dong|nghiem-thu|giam-sat|nhat-ky|kinh-nghiem|chon-thau|cach-chon/.test(s))
    return "process";
  if (/vat-tu|vat-lieu|tiet-kiem|an-toan|ben-dep|bao-hanh|khong-ban|truc-tiep|doi-thi-cong/.test(s))
    return "turnkey";
  return "turnkey";
}
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/quan-1|quận 1/.test(s)) return "Quận 1, TP.HCM";
  if (/quan-3|quận 3/.test(s)) return "Quận 3, TP.HCM";
  if (/quan-7|quận 7/.test(s)) return "Quận 7, TP.HCM";
  if (/quan-9|quận 9/.test(s)) return "Quận 9 / TP. Thủ Đức";
  if (/quan-10|quận 10/.test(s)) return "Quận 10, TP.HCM";
  ...
  if (/can-gio|cần giờ/.test(s)) return "Cần Giờ, TP.HCM";
  ...
  if (/quan-12|quận 12/.test(s)) return "Quận 12, TP.HCM";
  if (/tuy-hoa|tuy hòa/.test(s)) return "Tuy Hòa, Phú Yên";
  if (/phu-yen|phú yên/.test(s)) return "Phú Yên";
  if (/dak-lak|đắk lắk/.test(s)) return "Đắk Lắk";
  if (/buon-ma-thuot|buôn ma thuột/.test(s)) return "Buôn Ma Thuột, Đắk Lắk";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch17
  ...
  const fallback = shortKeywordCalendarBatch17
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch17.map((item) => {
...
export const shortKeywordBatch17ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch17Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch17Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch17ArticlesBySlug[slug];
}
