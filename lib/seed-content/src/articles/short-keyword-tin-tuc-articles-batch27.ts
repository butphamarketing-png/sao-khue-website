/**
 * 100 bài tin tức SEO — batch 27 từ khóa ngắn (#2147–#2246).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: dịch vụ xây dựng chuyên biệt, hạng mục kỹ thuật chuyên sâu.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch27 } from "../short-keyword-calendar-batch27";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch27
  ...
  const fallback = shortKeywordCalendarBatch27
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch27.map((item) => {
...
export const shortKeywordBatch27ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch27Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch27Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch27ArticlesBySlug[slug];
}
