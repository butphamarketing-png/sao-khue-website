/**
 * 100 bài tin tức SEO — batch 47 từ khóa ngắn (#4147–#4246).
...
import { shortKeywordCalendarBatch47 } from "../short-keyword-calendar-batch47";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch47
  ...
  const fallback = shortKeywordCalendarBatch47
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch47.map((item) => {
...
export const shortKeywordBatch47ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch47Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch47Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch47ArticlesBySlug[slug];
}
