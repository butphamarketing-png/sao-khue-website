/**
 * 100 bài tin tức SEO — batch 19 từ khóa ngắn (#1347–#1446).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: kết cấu/móng, mái thoát nước, phòng chức năng, so sánh gói, geo vành đai, FAQ mua hàng.
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch21 } from "../short-keyword-calendar-batch21";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch21
  ...
  const fallback = shortKeywordCalendarBatch21
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch21.map((item) => {
...
export const shortKeywordBatch21ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch21Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch21Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch21ArticlesBySlug[slug];
}
