/**
 * 100 bài tin tức SEO — batch 63 từ khóa ngắn (#5747–#5846).
...
import { shortKeywordCalendarBatch63 } from "../short-keyword-calendar-batch63";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch63
  ...
  const fallback = shortKeywordCalendarBatch63
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch63.map((item) => {
...
export const shortKeywordBatch63ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch63Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch63Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch63ArticlesBySlug[slug];
}
