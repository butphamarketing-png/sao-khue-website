/**
 * 100 bài tin tức SEO — batch 52 từ khóa ngắn (#4647–#4746).
...
import { shortKeywordCalendarBatch52 } from "../short-keyword-calendar-batch52";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch52
  ...
  const fallback = shortKeywordCalendarBatch52
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch52.map((item) => {
...
export const shortKeywordBatch52ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch52Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch52Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch52ArticlesBySlug[slug];
}
