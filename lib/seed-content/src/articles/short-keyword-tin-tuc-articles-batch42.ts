/**
 * 100 bài tin tức SEO — batch 42 từ khóa ngắn (#3647–#3746).
...
import { shortKeywordCalendarBatch42 } from "../short-keyword-calendar-batch42";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch42
  ...
  const fallback = shortKeywordCalendarBatch42
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch42.map((item) => {
...
export const shortKeywordBatch42ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch42Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch42Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch42ArticlesBySlug[slug];
}
