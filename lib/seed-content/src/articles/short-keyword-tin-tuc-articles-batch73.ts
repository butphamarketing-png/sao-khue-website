/**
 * 100 bài tin tức SEO — batch 73 từ khóa ngắn (#6747–#6846).
 * Chuẩn: 1500+ từ, ≥6 H2, 3 ảnh (alt = từ khóa), FAQ, bảng giá, internal link.
 * Cluster: Long-tail thương mại TP.HCM + tỉnh lân cận (indexable *-sk73).
 */
import {
  buildCtrExcerpt,
...
import { shortKeywordCalendarBatch73 } from "../short-keyword-calendar-batch73";
...
function detectLocation(slug: string, keyword: string): string {
  ...
  if (/quan-10|cho-ray|le-thi-rieng/.test(s)) return "Quận 10, TP.HCM";
...
function clusterLinksSection(b: Brief): string {
  const kw = b.focusKeyword;
  const related = shortKeywordCalendarBatch73
  ...
  const fallback = shortKeywordCalendarBatch73
    .filter((item) => item.slug !== b.slug)
    .slice(0, 4);
...
const briefs: Brief[] = shortKeywordCalendarBatch73.map((item) => {
...
export const shortKeywordBatch73ArticlesBySlug: Record<string, SeoArticleShape> = Object.fromEntries(
  briefs.map((b) => [b.slug, buildArticle(b)]),
);

export const shortKeywordBatch73Slugs = briefs.map((b) => b.slug);

export function getShortKeywordBatch73Article(slug: string): SeoArticleShape | undefined {
  return shortKeywordBatch73ArticlesBySlug[slug];
}
