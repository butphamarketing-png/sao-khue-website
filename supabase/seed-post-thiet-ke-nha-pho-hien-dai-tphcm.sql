-- Sao Khuê — Thiết Kế Nhà Phố Hiện Đại Đẹp Tại TP.HCM
-- UTF-8 — Supabase SQL Editor → paste TOÀN BỘ file → Run
-- Kiểm tra: SELECT slug, title FROM posts WHERE slug = 'thiet-ke-nha-pho-hien-dai-tphcm';
...
<p><strong>Kiến Trúc Sao Khuê</strong> — <strong>Thiết kế nhà phố TP.HCM</strong> chuẩn kỹ thuật, thẩm mỹ hiện đại, sẵn sàng đồng hành từ bản vẽ đến công trình hoàn thiện.</p>$content$,
  ...
  $alt$thiết kế nhà phố tphcm$alt$,
  $cap$thiết kế nhà phố tphcm$cap$,
